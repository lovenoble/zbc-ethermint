...
func (egcd EthGasConsumeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	shangai := ethCfg.IsShanghai(blockHeight, uint64(ctx.BlockTime().Unix()))
	...
	for _, msg := range tx.GetMsgs() {
		...
		fees, err := keeper.VerifyFee(txData, evmDenom, baseFee, homestead, istanbul, shangai, ctx.IsCheckTx())
		if err != nil {
